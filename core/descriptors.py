...
class MPNNDescriptorCalculator:
    ...
    def _calculate_single(self, mol) -> dict[str, float]:
        ...
        try:
            ...
            desc["halogen_fraction"] = (
                total_halogens / desc["heavy_atom_count"] if desc["heavy_atom_count"] > 0 else 0
            )
            ...
            # Derived
            desc["logd_proxies"] = (
                desc["logp"] - 0.5 * desc["num_basic_N"] + 0.3 * desc["num_acidic_O"]
            )

            num_aromatic_atoms = sum(1 for atom in mol.GetAtoms() if atom.GetIsAromatic())
            desc["aromatic_proportion"] = (
                num_aromatic_atoms / desc["heavy_atom_count"] if desc["heavy_atom_count"] > 0 else 0
            )
            ...
            desc["polar_surface_ratio"] = (
                desc["tpsa"] / desc["labute_asa"] if desc["labute_asa"] > 0 else 0
            )
            ...
            if desc["logp"] > 4:
                cyp_alerts += 1
            if desc["aromatic_ring_count"] > 3:
                cyp_alerts += 1
            desc["cyp_substrate_alerts"] = cyp_alerts
            desc["oxidation_susceptibility"] = (
                desc["num_allylic_sites"]
            ...
            aromatic_carbons = sum(
                1 for atom in mol.GetAtoms() if atom.GetAtomicNum() == 6 and atom.GetIsAromatic()
            )
            ...
            desc["agp_binding_score"] = (
                desc["logp"] + desc["num_basic_groups"] * 1.5 + desc["num_tertiary_amine"]
            )
            desc["cad_score"] = (
                desc["logp"]
                + (desc["num_basic_groups"] + desc["num_tertiary_amine"]) * 2
                - desc["num_acidic_groups"]
            )
            desc["protein_binding_proxy"] = max(
                desc["albumin_binding_score"], desc["agp_binding_score"]
            )
            ...
            desc["LE_proxy"] = (
                desc["logp"] / desc["heavy_atom_count"] if desc["heavy_atom_count"] > 0 else 0
            )
            ...
            desc["esol_logS"] = (
                ...
                - 0.63 * desc["logp"]
                - 0.0062 * desc["mw"]
                + 0.066 * desc["rotatable"]
                - 0.74 * desc["aromatic_proportion"]
            )
            ...
            desc["sp2_fraction"] = (
                sp2_atoms / desc["heavy_atom_count"] if desc["heavy_atom_count"] > 0 else 0
            )
            desc["aromatic_density"] = (
                num_aromatic_atoms / desc["heavy_atom_count"] if desc["heavy_atom_count"] > 0 else 0
            )
...
class CacoERFeatureGenerator:
    ...
    def _get_descriptors(self, mol) -> list[float]:
        ...
        try:
            ...
            basic_n = sum(
                [
                    len(mol.GetSubstructMatches(self.smarts[k]))
