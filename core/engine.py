...
from config.settings import PREDICTION_CONSTRAINTS, TRAINED_MODELS_DIR
        ...
        try:
            ...
            if training_style == "optimized_caco_er":
                ...
                if predictor.load_models():
                    ...
                    print("    ✓ Loaded Caco-ER model")
                    return True
                else:
                    print("    ✗ Failed to load Caco-ER model")
                    return False
                ...
                if predictor.load_models():
                    self.predictors[target] = predictor
                    self.loaded_targets.add(target)
                    print("    ✓ Loaded Script1 model")
                    return True
                else:
                    print("    ✗ Failed to load Script1 model")
                    return False
                ...
                if predictor.load_models():
                    self.predictors[target] = predictor
                    self.loaded_targets.add(target)
                    print("    ✓ Loaded Script2 model")
                    return True
                else:
                    print("    ✗ Failed to load Script2 model")
                    return False
                ...
                predictor = MPNNPredictorHybridV5Refinement(
                    model_dir=model_dir, checkpoint_version=checkpoint_version
                )
                if predictor.load_models():
                    self.predictors[target] = predictor
                    self.loaded_targets.add(target)
                    print("    ✓ Loaded HybridV5 Refinement model")
                    return True
                else:
                    print("    ✗ Failed to load HybridV5 Refinement model")
                    return False
                ...
                if predictor.load_models():
                    self.predictors[target] = predictor
                    self.loaded_targets.add(target)
                    print("    ✓ Loaded HybridV5 Integrated model")
                    return True
                else:
                    print("    ✗ Failed to load HybridV5 Integrated model")
                    return False
