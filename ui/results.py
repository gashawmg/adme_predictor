...
def create_download_button(results_df: pd.DataFrame, filename: str = "predictions.csv"):
    ...
    st.download_button(
        label="📥 Download Results (CSV)", data=csv_data, file_name=filename, mime="text/csv"
    )


def display_molecule_results(
    smiles: str, predictions: dict[str, float], index: int, show_log: bool = False
):
    """Display predictions for a single molecule."""
    from config.conversion_config import convert_log_to_actual
            ...
            if config.get("log_scale", False):
                ...
                if unit:
                    st.metric(
                        ...
                        help=f"Unit: {unit}"
                        + (f" | Log value: {log_value:.4f}" if show_log else ""),
                    )
                else:
                    st.metric(
                        label=display_name,
