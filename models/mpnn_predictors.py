...
class BaseMPNNPredictor:
    ...
    def _load_checkpoint_safe(self, path: str) -> MPNN | None:
...
class MPNNPredictorScript1(BaseMPNNPredictor):
    ...
    def __init__(self, model_dir: str, use_refinement: bool = False, checkpoint_version="latest"):
    ...
    def predict(self, smiles_list: list[str]) -> np.ndarray:
        ...
        for i, smi in enumerate(smiles_list):
            ...
            if mol:
                desc = (
                    integrated_descs[i].astype(np.float32) if integrated_descs is not None else None
                )
...
class MPNNPredictorScript2(BaseMPNNPredictor):
    ...
    def predict(self, smiles_list: list[str]) -> np.ndarray:
        ...
        integrated_descs = (
            self.desc_calc_integrated.transform(smiles_list) if self.desc_calc_integrated else None
        )
